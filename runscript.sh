...
proceed_if_not_exists "${MODEL_DIR}/era5_init.grib" "python src/download_era5.py \
    $DATE_TIME $END_DATE_TIME $INTERVAL $MODEL_NAME"
proceed_if_not_exists "${MODEL_DIR}/ifs_ens.zarr/.zmetadata" "python src/download_ifs.py \
...
python -u src/create_zarr.py "$MODEL_DIR"
...
for MEMBER in $(seq 0 $((NUM_MEMBERS - 1))); do
    ...
    if [ "$(echo "$PERTURBATION_INIT > 0.0" | bc -l)" -eq 1 ]; then
        proceed_if_not_exists "${MEMBER_DIR}/era5_init.grib" \
            "python -u ${BASE_DIR}/src/perturb_era5.py $DATE_TIME $MODEL_NAME \
    ...
    if [ "$(echo "$PERTURBATION_LATENT > 0.0" | bc -l)" -eq 1 ]; then
        if [ "$MODEL_NAME" = "graphcast" ]; then
            if [ ! -d "${MEMBER_DIR}/params" ]; then
                python -u "${BASE_DIR}/src/perturb_graphcast.py" "$DATE_TIME" "$MODEL_NAME" \
                    "$PERTURBATION_INIT" "$PERTURBATION_LATENT" "$MEMBER"
            fi
        else
            proceed_if_not_exists "${MEMBER_DIR}/weights.tar" \
                "python -u ${BASE_DIR}/src/perturb_fourcastnet.py $DATE_TIME $MODEL_NAME \
                $PERTURBATION_INIT $PERTURBATION_LATENT $MEMBER"
        fi
    else
        if [ "$MODEL_NAME" = "graphcast" ]; then
...
python -u src/create_zarr.py "$PERTURBATION_DIR" --subdir_search True

# if ! test -d "${REGION_DIR}/png_${MODEL_NAME}"; then
    echo "Evaluating model and generating figures"
    python -u src/evaluation.py "$DATE_TIME" "$MODEL_NAME" "$PERTURBATION_INIT" \
...
if [ -z "$(find "${PERTURBATION_DIR}/0/${CROP_REGION}/animations/" -name '*gif' -print -quit 2>/dev/null)" ]; then
    ...
    python -u src/animator.py "$DATE_TIME" "$MODEL_NAME" "$PERTURBATION_INIT" "$PERTURBATION_LATENT" "$CROP_REGION"
    # python -u src/animator_3d.py "$DATE_TIME" "$MODEL_NAME" "$PERTURBATION_INIT" "$PERTURBATION_LATENT" "$CROP_REGION"
fi
