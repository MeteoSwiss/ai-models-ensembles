...
def perturb_weights(params, perturbation_strength):
    """Perturb the weights of layer 7 in the processor of the GraphCast model."""
    ...
    for key in keys_to_perturb:
        if key in params:
            ...
            print(
                f"  Range: [{params[key].min():.4f}, {params[key].max():.4f}]")
...
def main():
    ...
    print(
        f"Perturbing the weights in layer 7 of the processor by {args.perturbation_latent}")
