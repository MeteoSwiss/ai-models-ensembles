#!/usr/bin/env bash
# Multi-lead ES / VS / SIGK under the FIXED 1990-2019 climatological channel
# scale (Fuhrer review item 2: the per-init truth std is truth-dependent and
# costs ES/VS/SIGK their propriety). Same run set as
# submit_table_metrics_multilead.sh so tab:calibration and Tab. C1 can be
# regenerated wholesale; only the standardisation differs.
...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --job-name=tbl_fixedscale
#SBATCH --output=/iopsstor/scratch/cscs/sadamov/tbl_fixedscale_%j.log
...
esvs () {  # label  lead  zarr-glob
    ...
    $PY -u tools/energy_variogram_score.py --forecast-zarrs $* \
        ...
        --lead "$L" --model-label "$lbl" --scale-json "$SCALE" \
        --out-csv "$OUT/esvs_${lbl}_fixed_L${L}.csv" > "$OUT/esvs_${lbl}_fixed_L${L}.log" 2>&1
}
sigk () {  # label  lead  zarr-glob
    ...
    $PY -u tools/signature_kernel_score.py --forecast-zarrs $* \
        ...
        --model-label "$lbl" --scale-json "$SCALE" \
        --out-csv "$OUT/sigk_${lbl}_fixed_L${L}.csv" > "$OUT/sigk_${lbl}_fixed_L${L}.log" 2>&1
}
