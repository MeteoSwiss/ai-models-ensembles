...
# Published verified ES / VS / SIGK @ 240 h (do NOT recompute).
ES_240 = {
    "aifsens": 1.018,
    "atlas": 1.038,
    "aifs_perturbed": 1.050,
    "fcn3": 1.061,
    "graphcast_all": 1.111,
...
def main():
    ...
    L.append("           mean (3D variables averaged over $\\ell\\in\\{500,850\\}$\\,hPa first; LSD over")
    ...
    L.append("        & \\multicolumn{6}{c}{\\textbf{120\\,h}}")
    L.append("        & \\multicolumn{6}{c}{\\textbf{240\\,h}}")
    L.append("        & \\multicolumn{3}{c}{\\textbf{240\\,h mvar}} \\\\")
    L.append("    \\cmidrule(lr){3-8} \\cmidrule(lr){9-14} \\cmidrule(lr){15-17}")
    L.append("     &  & CRPSS$\\uparrow$ & SSR & SSIM$\\uparrow$ & LSD$\\downarrow$ & FSS$\\uparrow$ & $W_1\\downarrow$")
    L.append("        & CRPSS$\\uparrow$ & SSR & SSIM$\\uparrow$ & LSD$\\downarrow$ & FSS$\\uparrow$ & $W_1\\downarrow$")
    L.append("        & ES$\\downarrow$ & VS$\\downarrow$ & SIGK \\\\")
    L.append("    \\midrule")
    ...
    for g in GROUP_ORDER:
        ...
        L.append(f"    \\multicolumn{{17}}{{@{{}}l}}{{\\textbf{{{ROLE_LABEL[g]}}}}} \\\\")
        for m in members:
            c120 = M[m][120]
            c240 = M[m][240]
            es = fmv(ES_240.get(m))
            vs = fmv(VS_240.get(m))
            sk = SIGK_240.get(m)
            es_s = "-" if es is None else f"{es:.3f}"
            vs_s = "-" if vs is None else f"{vs:.4f}"
            sk_s = "-" if sk is None else f"{sk:.1f}"
            cells = [
                f3(c120["CRPSS"]), f2(c120["SSR"]), f3(c120["SSIM"]),
                f3(c120["LSD"]), f3(c120["FSS"]), fw1(c120["W1"]),
                f3(c240["CRPSS"]), f2(c240["SSR"]), f3(c240["SSIM"]),
                f3(c240["LSD"]), f3(c240["FSS"]), fw1(c240["W1"]),
                es_s, vs_s, sk_s,
            ]
