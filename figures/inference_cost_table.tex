...
\begin{table}[t]
  ...
  the AIFS v1 deterministic checkpoint at the per-phase winner
  identified by AIFS Phase 1 + Phase 2 ablations (verified 2026-06-01):
  Phase 1 \texttt{mag\_0.01\_layer\_all} chosen as the production baseline
  on the same multi-metric rule as GraphCast (Phase 2
  \texttt{decoder@0.0275} wins CRPS at all leads but is over-dispersed at
  SSR 1.23 vs Phase 1's 0.83 at lead 240 h). Production-baseline run
  (1120 forecasts) is pending; the per-member, CS h, NH, CHF, kWh values
  here are estimates from the AIFS-CRPS architectural twin (same forward
  graph, parameters perturbed but not retrained).
