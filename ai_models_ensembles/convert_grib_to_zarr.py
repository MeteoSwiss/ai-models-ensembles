...
def convert_grib_to_zarr(path_out: str, subdir_search: bool = False) -> None:
    ...
    path_store = os.path.join(path_out, "forecast.zarr")
    ...
    if os.path.exists(path_store):
        ...
        print(
            f"Found existing zarr store at: {path_store} with members: {zarr_store.member.values}"
        )
    else:
